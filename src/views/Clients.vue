  ...
  <v-row>
    <v-col cols="12">
      <v-data-table
        :headers="headers"
        :items="
          filterSettings.enabled ? filterSettings.filteredClients : clients
        "
        :hide-default-footer="
          filterSettings.enabled
            ? filterSettings.filteredClients.length <= 10
            : clients.length <= 10
        "
        ...
        <template v-slot:item.inbounds="{ item }">
          <span>
            <v-tooltip
              ...
              v-if="item.inbounds != ''"
            >
              <span v-for="i in item.inbounds"
                >{{ inbounds.find((inb) => inb.id == i)?.tag }}<br
              /></span>
            </v-tooltip>
        ...
        <template v-slot:item.volume="{ item }">
          ...
          >
            <v-chip
              size="small"
              :color="
                item.volume > 0 && item.volume <= item.up + item.down
                  ? 'error'
                  : ''
              "
              label
              >{{
                item.volume == 0
                  ? $t("unlimited")
                  : HumanReadable.sizeFormat(item.volume)
              }}</v-chip
            >
          </div>
          <v-progress-linear
        ...
        <template v-slot:item.expiry="{ item }">
          <div class="text-start">
            <v-chip
              size="small"
              :color="
                item.expiry > 0 && item.expiry <= Date.now() / 1000
                  ? 'error'
                  : ''
              "
              label
            ...
            <v-card :title="$t('actions.del')" rounded="lg">
              ...
              <v-card-actions>
                ...
                <v-btn
                  ...
                  @click="
                    delOverlay[
                      clients.findIndex((c) => c.id == item.id)
                    ] = false
                  "
                  >{{ $t("no") }}</v-btn
                >
...
const groups = computed((): string[] => {
  ...
  if (filterSettings?.value.enabled)
    return Array.from(
      new Set(filterSettings.value.filteredClients?.map((c) => c.group))
    );
...
const headers = [
  ...
  { title: i18n.global.t("pages.inbounds"), key: "inbounds", width: 120 },
  ...
  {
    title: i18n.global.t("actions.action"),
    key: "actions",
    sortable: false,
...
const doFilter = () => {
  ...
  if (filterSettings.value.group != "-") {
    filteredClients = filteredClients.filter(
      (c) => c.group == filterSettings.value.group
    );
  }
  if (filterSettings.value.text.length > 0) {
    const txt = filterSettings.value.text;
    filteredClients = filteredClients.filter(
      (c) => c.name.search(txt) != -1 || c.desc.search(txt) != -1
    );
  }
  switch (filterSettings.value.state) {
    ...
    case "expired":
      filteredClients = filteredClients.filter(
        (c) => c.expiry > 0 && c.expiry < Date.now() / 1000
      );
      break;
    case "online":
      filteredClients = filteredClients.filter((c) =>
        Data().onlines?.user?.includes(c.name)
      );
      break;
  }
